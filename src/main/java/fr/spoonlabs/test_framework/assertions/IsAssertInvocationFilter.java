package fr.spoonlabs.test_framework.assertions;
...
public class IsAssertInvocationFilter {
	...
	/**
	 * This field represent the number of method call to explore to find any
	 * assertion. i.e. it will match a chain of method calls as follow: m1() -> m2()
	 * -> .... -> mdepth(), where only mdepth() contains an invocation to an
	 * assertion.
	 */
	private final int depth;

	/**
	 * @param assertionClassesName the name of the classes of the assertions used,
	 *                             e.g. org.junit.Assert.
	 */
	...
	/**
	 * @param assertionClassesName the name of the class of the assertions used,
	 *                             e.g. org.junit.Assert.
	...
	public boolean isAssert(CtInvocation<?> invocation) {
		return _isAssert(invocation) || (invocation.getExecutable().getDeclaration() != null && !invocation
				.getExecutable().getDeclaration().getElements(new HasAssertInvocationFilter(this.depth)).isEmpty());
	}
	...
	private class HasAssertInvocationFilter extends TypeFilter<CtInvocation<?>> {
		...
		public boolean matches(CtInvocation<?> element) {
			return deep >= 0 && (_isAssert(element) || containsMethodCallToAssertion(element, this.deep));
		}
	}
	...
	private boolean _isAssert(CtInvocation<?> invocation) {
		...
		if (invocation.getExecutable().getDeclaringType().getPackage() == null) {
			if (invocation.getExecutable().getDeclaringType().getTopLevelType() != null) {
				qualifiedNameOfDeclaringType = invocation.getExecutable().getDeclaringType().getTopLevelType()
						.getQualifiedName();
			} else {
				return false;
			}
		...
		return this.assertionClassesName.stream().anyMatch(assertionClassName -> Pattern.compile(assertionClassName)
				.matcher(qualifiedNameOfDeclaringType).matches());
	}
	...
	private boolean containsMethodCallToAssertion(CtInvocation<?> invocation, int deep) {
		...
		final CtMethod<?> method = invocation.getExecutable().getDeclaringType().getTypeDeclaration().getMethod(
				invocation.getExecutable().getType(), invocation.getExecutable().getSimpleName(),
